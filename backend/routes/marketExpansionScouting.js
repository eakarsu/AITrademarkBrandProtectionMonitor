...
// Market expansion scouting identifying infringements in new geographies
// Feature: market-expansion-scouting
...
router.post('/analyze', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      'You are an expert assistant for the "AITrademarkBrandProtectionMonitor" platform. Always return strict JSON only — no markdown, no commentary.',
      `Feature: Market expansion scouting identifying infringements in new geographies.\nUser input: ${JSON.stringify(payload).slice(0, 3500)}\n\nProduce JSON in this shape:\n{\n  "summary": "...",\n  "findings": [...],\n  "recommendations": [...],\n  "score": 0.0,\n  "details": {}\n}`
    );
    const uid = req.user && (req.user.id || req.user.userId || req.user.user_id);
    await persist(uid, 'market-expansion-scouting', payload, result);
    res.json({ success: true, feature: 'market-expansion-scouting', ...result });
  } catch (err) {
    console.error('market-expansion-scouting analyze error:', err.message);
...
router.post('/batch', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      'You analyze batches of items for "AITrademarkBrandProtectionMonitor". Return strict JSON only.',
      `Feature: Market expansion scouting identifying infringements in new geographies. Batch of ${items.length} items.\nItems: ${JSON.stringify(items).slice(0, 3500)}\nReturn JSON: { results:[{index, score, summary, recommendation}], aggregate:{count, mean_score, top_concerns:[]} }`,
      { maxTokens: 3072 }
    );
    res.json({ success: true, feature: 'market-expansion-scouting', count: items.length, ...result });
  } catch (err) {
...
router.get('/info', auth, (req, res) => {
  res.json({ feature: 'market-expansion-scouting', title: 'Market expansion scouting identifying infringements in new geographies', project: 'AITrademarkBrandProtectionMonitor' });
});
