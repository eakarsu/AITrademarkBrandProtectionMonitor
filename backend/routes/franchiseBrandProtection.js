...
// Franchise brand protection monitoring franchisee compliance
// Feature: franchise-brand-protection
...
router.post('/analyze', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Franchise brand protection monitoring franchisee compliance.\nUser input: ${JSON.stringify(payload).slice(0, 3500)}\n\nProduce JSON in this shape:\n{\n  "summary": "...",\n  "findings": [...],\n  "recommendations": [...],\n  "score": 0.0,\n  "details": {}\n}`
    );
    const uid = req.user && (req.user.id || req.user.userId || req.user.user_id);
    await persist(uid, 'franchise-brand-protection', payload, result);
    res.json({ success: true, feature: 'franchise-brand-protection', ...result });
  } catch (err) {
    console.error('franchise-brand-protection analyze error:', err.message);
...
router.post('/batch', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Franchise brand protection monitoring franchisee compliance. Batch of ${items.length} items.\nItems: ${JSON.stringify(items).slice(0, 3500)}\nReturn JSON: { results:[{index, score, summary, recommendation}], aggregate:{count, mean_score, top_concerns:[]} }`,
      { maxTokens: 3072 }
    );
    res.json({ success: true, feature: 'franchise-brand-protection', count: items.length, ...result });
  } catch (err) {
...
router.get('/history', auth, async (req, res) => {
  try {
    ...
    const { rows } = await pool.query(
      ...
      [uid, 'franchise-brand-protection']
    );
...
router.get('/info', auth, (req, res) => {
  res.json({ feature: 'franchise-brand-protection', title: 'Franchise brand protection monitoring franchisee compliance', project: 'AITrademarkBrandProtectionMonitor' });
});
