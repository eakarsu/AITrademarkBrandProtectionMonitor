...
// Visual counterfeit detection from product photos
// Feature: visual-counterfeit-detection
...
router.post('/analyze', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Visual counterfeit detection from product photos.\nUser input: ${JSON.stringify(payload).slice(0, 3500)}\n\nProduce JSON in this shape:\n{\n  "summary": "...",\n  "findings": [...],\n  "recommendations": [...],\n  "score": 0.0,\n  "details": {}\n}`
    );
    const uid = req.user && (req.user.id || req.user.userId || req.user.user_id);
    await persist(uid, 'visual-counterfeit-detection', payload, result);
    res.json({ success: true, feature: 'visual-counterfeit-detection', ...result });
  } catch (err) {
    console.error('visual-counterfeit-detection analyze error:', err.message);
...
router.post('/batch', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Visual counterfeit detection from product photos. Batch of ${items.length} items.\nItems: ${JSON.stringify(items).slice(0, 3500)}\nReturn JSON: { results:[{index, score, summary, recommendation}], aggregate:{count, mean_score, top_concerns:[]} }`,
      { maxTokens: 3072 }
    );
    res.json({ success: true, feature: 'visual-counterfeit-detection', count: items.length, ...result });
  } catch (err) {
...
router.get('/history', auth, async (req, res) => {
  try {
    ...
    const { rows } = await pool.query(
      ...
      [uid, 'visual-counterfeit-detection']
    );
...
router.get('/info', auth, (req, res) => {
  res.json({ feature: 'visual-counterfeit-detection', title: 'Visual counterfeit detection from product photos', project: 'AITrademarkBrandProtectionMonitor' });
});
