...
export default function GapNoDirectIntegrationWithUsptoWipoDatabases() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/gap-no-direct-integration-with-uspto-wipo-databases-only/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'gap-no-direct-integration-with-uspto-wipo-databases-only', project: 'AITrademarkBrandProtectionMonitor' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/gap-no-direct-integration-with-uspto-wipo-databases-only/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">No direct integration with USPTO, WIPO databases (only general integration module)</h1>
        <p className="text-slate-500 text-sm mt-1">Batch 08 · gap_non_ai · AITrademarkBrandProtectionMonitor</p>
      </div>
