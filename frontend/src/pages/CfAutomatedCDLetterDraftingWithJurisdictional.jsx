...
// Feature: Automated C&D letter drafting with jurisdictional legal language
...
export default function CfAutomatedCDLetterDraftingWithJurisdictional() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-automated-c-d-letter-drafting-with-jurisdictional-legal/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-automated-c-d-letter-drafting-with-jurisdictional-legal', project: 'AITrademarkBrandProtectionMonitor' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-automated-c-d-letter-drafting-with-jurisdictional-legal/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Automated C&D letter drafting with jurisdictional legal language</h1>
