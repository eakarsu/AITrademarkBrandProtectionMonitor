...
export default function CfMarketExpansionScoutingIdentifyingInfringementsInNew() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-market-expansion-scouting-identifying-infringements-in-new-geographies/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-market-expansion-scouting-identifying-infringements-in-new-geographies', project: 'AITrademarkBrandProtectionMonitor' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-market-expansion-scouting-identifying-infringements-in-new-geographies/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Market expansion scouting identifying infringements in new geographies</h1>
        <p className="text-slate-500 text-sm mt-1">Batch 08 · cfs · AITrademarkBrandProtectionMonitor</p>
      </div>
