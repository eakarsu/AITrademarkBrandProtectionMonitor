...
export default function CfFranchiseBrandProtectionMonitoringFranchiseeCompliance() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-franchise-brand-protection-monitoring-franchisee-compliance/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-franchise-brand-protection-monitoring-franchisee-compliance', project: 'AITrademarkBrandProtectionMonitor' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-franchise-brand-protection-monitoring-franchisee-compliance/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Franchise brand protection monitoring franchisee compliance</h1>
