...
export default function CfVisualCounterfeitDetectionFromProductPhotos() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-visual-counterfeit-detection-from-product-photos/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-visual-counterfeit-detection-from-product-photos', project: 'AITrademarkBrandProtectionMonitor' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-visual-counterfeit-detection-from-product-photos/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Visual counterfeit detection from product photos</h1>
