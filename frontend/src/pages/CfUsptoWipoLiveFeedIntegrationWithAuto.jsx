...
export default function CfUsptoWipoLiveFeedIntegrationWithAuto() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-uspto-wipo-live-feed-integration-with-auto-renewal-tracking/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-uspto-wipo-live-feed-integration-with-auto-renewal-tracking', project: 'AITrademarkBrandProtectionMonitor' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-uspto-wipo-live-feed-integration-with-auto-renewal-tracking/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">USPTO/WIPO live feed integration with auto-renewal tracking</h1>
